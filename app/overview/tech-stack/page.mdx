...
Optional component-library browser. Coreola reserves `npm run storybook` and `npm run build-storybook` scripts, but Storybook packages and `.storybook/` configuration are not initialized by default. See [Storybook](/development/storybook) before using those scripts.
