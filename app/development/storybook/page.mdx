...
Coreola has **Storybook configured and ready to run**. `package.json` contains `npm run storybook` and `npm run build-storybook`, `.storybook/` contains the shared configuration, and component stories live under `src/components/`.

This page covers what is in place, how the setup is wired, and the conventions to apply when adding stories.
...
The default dev-server port is **6006** at `http://localhost:6006`.

Storybook uses the Vite framework package, `addon-docs`, `addon-a11y`, and `addon-themes` so it stays aligned with the app's Vite 7 toolchain.
...
const config: StorybookConfig = {
  stories: ['../src/**/*.stories.@(ts|tsx|mdx)'],
  addons: [
    '@storybook/addon-docs',        // generated docs and autodocs pages
...
- A story for **interactive** scenarios using `storybook/test` if the component has non-trivial behavior.
