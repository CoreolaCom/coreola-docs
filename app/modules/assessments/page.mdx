...
- `status` — workflow status (e.g., `needs_review`, `waiting_for_evidence`, `remediation_in_progress`, `blocked`, `overdue`, `closed`)
- `priority`, `risk_level`, `decision`
- `owner`, `reviewer`, `customer` — related entities
- `findings_count`, `evidence_requests_count`
- `due_date`, `tags`, `control_sets`

#### `AssessmentFinding`

A finding raised against an assessment.

- `id`, `assessment_id`, `title`, `category`
- `severity`, `status` (`open` / `in_progress` / `resolved` / `closed`)
- `owner_id`, `due_date`
...
- `id`, `assessment_id`, `title`
- `status` (`requested` / `in_review` / `received` / `rejected`)
- `recipient_id`, `due_date`, `file_count`
